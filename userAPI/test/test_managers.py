...
class TestModel(APITestCase):
    ...
    def test_create_user(self):
        user = User.objects.create_user("cryce", "crycetruly@gmail.com", "client", "Password@123")
    ...
    def test_raising_error_message_when_no_username_is_supplied(self):
        with self.assertRaisesMessage(ValueError, "The Username field must be set"):
            User.objects.create_user(username="", email="crycetruly@gmail.com", role="client", password="Password@123")
    ...
    def test_raising_error_message_when_no_email_is_supplied(self):
        with self.assertRaisesMessage(ValueError, "The Email field must be set"):
            User.objects.create_user(username="cryce", email="", role="client", password="Password@123")

              
    def test_create_super_user(self):
        user = User.objects.create_superuser("cryce", "crycetruly@gmail.com", "admin", "Password@123")
        self.assertIsInstance(user, User)
    ...
    def test_create_superuser_with_staff_status(self):
        with self.assertRaisesMessage(ValueError, "Superuser must have is_staff=True."):
            User.objects.create_superuser(username="cryce", email="crycetruly@gmail.com", role="admin", password="Password@123", is_staff=False)
            
            
    def test_create_superuser_with_superuser_status(self):
        with self.assertRaisesMessage(ValueError, "Superuser must have is_superuser=True."):
            User.objects.create_superuser(username="cryce", email="crycetruly@gmail.com", role="admin", password="Password@123", is_superuser=False)
